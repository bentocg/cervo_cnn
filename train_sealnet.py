...
def train_model(model, criterion1, criterion2, criterion3, optimizer, scheduler, bce_weight, fp_weight=0.05,
                fn_weight=0.15, num_epochs=25):
    """
    ...
    :param fp_weight: extra weight for every false-positive on training batch
    :param fn_weight: extra weight for every false-negative on training batch
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['training', 'validation']:
                ...
                for iter, data in enumerate(dataloaders[phase]):
                    ...
                    pred_xy = [get_xy_locs(loc, int(round(
                        out_dict['count'][idx].item()))) for idx, loc in
                               enumerate(out_dict['heatmap'].detach().cpu().numpy())]
                    ...
                    for idx, ele in enumerate(ground_truth_xy):
                        ...
                        if len(ele) == 0:
                            ...
                            fp_batch = len(pred_xy[idx]) - n_matches
                            fn_batch = len(ele) - n_matches
                    ...
                    for key in out_dict:
                        ...
                        # weight with false-positives and false-negatives
                        if key == "count":
                            batch_loss[key] *= (1 + fp_weight * fp_batch + fn_weight * fn_batch)
            ...
            for loss in epoch_loss:
                writer.add_scalar('{}_loss_{}'.format(phase, loss), epoch_loss[loss], global_step=global_step)
            writer.add_scalar('{}_precision'.format(phase), epoch_precision, global_step=global_step)
            writer.add_scalar('{}_recall'.format(phase), epoch_recall, global_step=global_step)
            writer.add_scalar('{}_f1'.format(phase), epoch_f1, global_step=global_step)
            if 'occupancy' in out_dict:
...
def main():
    ...
    # extract false-positive and false-negative weights
    fp_weight = hyperparameters[args.hyperparameter_set]['fp_weight']
    fn_weight = hyperparameters[args.hyperparameter_set]['fn_weight']
